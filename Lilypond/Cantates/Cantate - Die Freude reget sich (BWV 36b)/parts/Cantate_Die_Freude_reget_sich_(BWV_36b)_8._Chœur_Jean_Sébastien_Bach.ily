...
globalMarksBreaks = {
  ...
  s1*31
  \bar "||" \time 4/4
  s4 \mark \markup { \bold "RÉCITATIF" } s4 s2
  s1*5
  \partial 2 s2
  \override Score.BarNumber.stencil = #ii-bar-number
  \bar "||" \set Score.currentBarNumber = #38
  \time 2/2 \partial 2 \mark \markup { \bold "CHŒUR" } s2
  \revert Score.BarNumber.stencil
  s1*7
  \bar "||" \time 4/4 s4
  \mark \markup { \bold "RECITATIF" } s4 s2
  s1*3
  \partial 2 s2
  \override Score.BarNumber.stencil = #ii-bar-number
  ...
  \time 2/2 \partial 2 \mark \markup { \bold "CHŒUR" }
...
fluteMusic = \relative fis'' {
  ...
  g8( fis8) e8( d8) cis8( e8) d8( fis8) | % 96
   | % 96
  e8( g8) fis8( e8) cis4.\trill d8 | % 97
  | % 97
  d2\fermata \fine
}
...
figBassWarning = {
  ...
  \mark\markup {
    ...
    \column { \smaller {
      \line { "* La basse chiffrée dans cette pièce"
      "n'est pas présente dans le score original." } } }
  }
}
