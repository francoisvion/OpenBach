sopranoMusic = {
  \partial 4 b'4
  b' b' a' cis''
  cis'' b' b'\fermata b'
  b' a' b'8 [cis''] d''4
  a' a' fis'\fermata g'
  e' g' fis' e'
  fis' gis' a'\fermata a'
  a' a' b' g'
  a' g' fis'\fermata d'
  e' g' fis' e'8 [fis']
  g'4 e' d'\fermata a'
  a' a' b' g'
  a' g' fis'\fermata g'
  g' g' fis' e'
  g' e' d'\fermata a'
  a' a' b' g'
  a' g' fis'\fermata \fine
}

altoMusic = {
  \partial 4 fis'4
  fis' gis' cis' a'
  e'8 [fis'] g'4 fis'\fermata fis'
  g' fis'8 [e'] d' [e'] fis'4
  fis' e' d'\fermata d'
  e' cis' d' e'
  d' d' e'\fermata e'
  fis' fis' g'8 [fis'] e'4
  fis'8 [dis'] e'4 dis'\fermata b
  cis' b8 [cis'] d'4 cis'8 [d']
  e' [d'] cis'4 a\fermata e'
  fis' fis' g' b
  fis' e' dis'\fermata e'
  d' e' d' cis'
  e'8 [d'] cis'4 a\fermata e'
  fis'8 [g'] a' [fis'] g'4 d'8 [e']
  fis'4 e' d'\fermata \fine
}

tenorMusic = {
  \partial 4 d'4
  dis' eis' fis' fis'
  cis'8 [dis'] e'4 dis'\fermata d'
  cis' cis' b a
  d' cis' a\fermata b
  a a a a
  a b cis'\fermata cis'
  d' d' d' e'8 [d']
  c'4 b b\fermata fis
  a g a a
  b a8 [g] fis4\fermata a
  d' d' d' e'
  b b b\fermata b
  b a a a
  b a8 [g] fis4\fermata a
  d' d'8 [c'] b [cis'] d'4
  d'8 [b] cis' [a] a4\fermata \fine
}

bassMusic = {
  \partial 4 b4
  a gis fis fis8 [gis]
  a4 e b\fermata b,
  e fis g fis8 [e]
  fis [g] a4 d\fermata b,
  cis a, d cis
  d8 [cis] b,4 a,\fermata a,
  d8 [e] fis [d] g4 c'8 [b]
  a4 e b,\fermata b
  a e fis8 [g] a4
  g a d\fermata cis
  d8 [c] b, [a,] g, [g] fis [e]
  dis4 e b,\fermata e
  b, cis d a
  g a d\fermata cis
  d8 [e] fis [d] g [a] b4
  a a, d\fermata \fine
}

sopranoLyrics = \lyricmode {
  Nun hilf uns, Herr, den Die -- nern dein,
  die mit deim Blut er -- lö -- set sein!
  Lass uns im Him -- mel ha -- ben teil
  mit den Heil -- gen im ew -- gen Heil.
  Hilf dei -- nem Volk, Herr Je -- su Christ,
  und seg -- ne, was dein Erb -- teil ist;
  wart und pfleg ihr zu al -- ler Zeit
  und heb sie hoch in E -- wig -- keit!
}

altoLyrics = \lyricmode {
  Nun hilf uns, Herr, den Die -- nern dein,
  die mit deim Blut er -- lö -- set sein!
  Lass uns im Him -- mel ha -- ben teil
  mit den Heil -- gen im ew -- gen Heil.
  Hilf dei -- nem Volk, Herr Je -- su Christ,
  und seg -- ne, was dein Erb -- teil ist;
  wart und pfleg ihr zu al -- ler Zeit
  und heb sie hoch in E -- wig -- keit!
}

tenorLyrics = \lyricmode {
  Nun hilf uns, Herr, den Die -- nern dein,
  die mit deim Blut er -- lö -- set sein!
  Lass uns im Him -- mel ha -- ben teil
  mit den Heil -- gen im ew -- gen Heil.
  Hilf dei -- nem Volk, Herr Je -- su Christ,
  und seg -- ne, was dein Erb -- teil ist;
  wart und pfleg ihr zu al -- ler Zeit
  und heb sie hoch in E -- wig -- keit!
}

bassLyrics = \lyricmode {
  Nun hilf uns, Herr, den Die -- nern dein,
  die mit deim Blut er -- lö -- set sein!
  Lass uns im Him -- mel ha -- ben teil
  mit den Heil -- gen im ew -- gen Heil.
  Hilf dei -- nem Volk, Herr Je -- su Christ,
  und seg -- ne, was dein Erb -- teil ist;
  wart und pfleg ihr zu al -- ler Zeit
  und heb sie hoch in E -- wig -- keit!
}
