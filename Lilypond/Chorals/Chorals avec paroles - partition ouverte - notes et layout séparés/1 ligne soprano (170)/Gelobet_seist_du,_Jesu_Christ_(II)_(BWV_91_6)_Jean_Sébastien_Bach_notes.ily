sopranoMusic = {
  \partial 4  g'4
  g' g' a' g'
  c'' d'' c''2\fermata
  b'8 [c''] d''4 e'' d''8 [c'']
  b'4 a' g'\fermata g'
  c'' b'8 [c''] d''4 a'
  g'8 [fis'] e'4 d'\fermata d'
  a' a' a' b'
  c''8 [b'] a'4 g'8 [a'] b'4
  a'2\fermata g'4 g'
  g'2 g'4\fermata \fine
}

altoMusic = {
  \partial 4  d'4
  e' d'8 [e'] fis'4 e'
  g' g'8 [f'] e'2\fermata
  g'8 [a'] b'4 c''8 [b'] a'4
  g' fis' d'\fermata d'8 [e']
  fis'4 g' g' fis'8 [e']
  d' [b] cis'4 a\fermata a
  d' cis'8 [a] fis'4 g'
  a'8 g'4 fis'8 e' [a'] d' [e']
  fis'2\fermata d'8 [e'] f'4
  e' e' d'\fermata \fine
}

tenorMusic = {
  \partial 4  b4
  c' b a8 [d'] b4
  e'8 [c'] b4 g2\fermata
  e'4 f' g' d'
  d'~ d'8 c' b4\fermata g
  a d'8 [c'] b [g] d'4
  d' a8 [g] fis4\fermata fis8 [g]
  a [b] cis' [e'] d'4 d'
  e'8 [d'] e' d'4 c'8 b [c']
  d'2\fermata d'4 b
  g c' b\fermata \fine
}

bassMusic = {
  \partial 4  g8 [fis]
  e [fis] g4 d e8 [d]
  c4 g, c2\fermata
  e4 d c fis,
  g,8 [b,] d4 g,\fermata b,
  a, g,8 [a,] b, [c] d [c]
  b, [g,] a,4 d\fermata d8 [e]
  fis [g] a [cis] d [c] b,4
  a,8 [b,] c [d] e [fis] g [g,]
  d2\fermata b,8 [a,] g,4
  c c g,\fermata \fine
}

sopranoLyrics = \lyricmode {
  Das hat er al -- les uns ge -- tan,
  sein groß Lieb zu zei -- gen an.
  des freu sich al -- le Chri -- sten -- heit
  und dank ihm des in E -- wig -- keit. __ _ _
  Ky -- ri -- e leis!
}

altoLyrics = \lyricmode {
  Das hat er al -- les uns ge -- tan, sein groß Lieb zu zei -- gen an. des freu sich al -- le Chri -- sten -- heit und dank ihm des in E -- wig -- _ keit. __ _ _ Ky -- ri -- e _ leis!
}

tenorLyrics = \lyricmode {
  Das hat er al -- les uns ge -- tan, sein groß Lieb zu zei -- gen an. des freu sich al -- le Chri -- sten -- heit und dank ihm des in E -- wig -- keit. __ _ _ _ Ky -- ri -- e _ leis!
}

bassLyrics = \lyricmode {
  Das hat er al -- les uns ge -- tan, sein groß Lieb zu zei -- gen an. des freu sich al -- le Chri -- sten -- heit und dank ihm des in E -- wig -- keit. __ _ _ Ky -- ri -- e _ leis!
}
