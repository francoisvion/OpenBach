\version "2.27.1"

#(set-global-staff-size 19)

\header {
 title = "Wie schön leuchtet der Morgenstern (choral)"
  opus = "BWV 436"
  composer = "Jean Sébastien Bach (1685-1750)"
  tagline = ##f

  }

\score {
  \new ChoirStaff <<
    \new Staff \with { \autoBeamOff instrumentName = \markup \center-column {S A} }
    {
      \clef treble
      \key e \major
      \time 4/4
      <<
        \new Voice = "soprano" {
          \voiceOne
          \partial 4  e'4
          b' gis'8 [fis'] e'4 b'
          cis'' cis'' b'\fermata b'
          cis'' dis'' e'' dis''
           cis'' cis'' b'\fermata gis'
          cis'' b' a' gis'
          fis'2 e'4\fermata \bar "|."
          \partial 4 r
          b'2 gis'\fermata
          b' gis'\fermata
          a'4 gis' fis' gis'
           a' gis' fis' gis'
          a' gis' fis'2
          e'\fermata e''4 dis''
          cis'' b' a' gis'
          fis'2 e'4\fermata \bar "|."
        }
        \new Voice = "alto" {
          \voiceTwo
          \partial 4  b4
          b b cis' dis'
          gis' fis' fis' gis'
          a' a' gis' fis'
           fis'8 [gis' fis' e'] dis'4 e'
          e' dis' e'8 [fis' gis' e']
          e'4 dis' b \bar "|."
          \partial 4 r
          fis'2 e'
          fis' e'
          fis'4 e' dis' e'
          fis' e' dis' e'
          fis' e'8 [dis'] cis' [dis'16 e'] dis'4
          b2 gis'4 fisis'
          gis' gis'8 [fis'] e'4 e'
          e' dis' b \bar "|."
        }
      >>
    }
    \new Lyrics \lyricsto "soprano" {
      Wie schön leuch -- tet der Mor -- gen -- stern woll Gnad' und Wahr -- heit von dem Herrn, die sü -- sse Wur -- zel Jes -- se;
      Lieb -- lich, freund -- lich, Schön und herr -- lich, Groß und ehr -- lich, Reich an Ga -- ben,
      Hoch und sehr präch -- tig er -- ha -- ben.
    }
    \new Staff \with { \autoBeamOff instrumentName = \markup \center-column {T B} }
    {
      \clef bass
      \key e \major
      \time 4/4
      <<
        \new Voice = "tenor" {
          \voiceOne
          \partial 4  gis4
          fis e8 [fis] gis [ais] b4
          b ais dis' e'
          e' fis' b b
           b ais b b
          a8 [gis] fis4 cis' b
          b4. a8 gis4 \bar "|."
          \partial 4 r
          b2 b
          dis' b
          cis'4 b b b
          b b b b
          cis'8 [dis'] e'4~ e'8 [cis' b a]
          gis2 cis'8 [b] ais4
          gis dis' cis' b
          cis' b8 [a] gis4 \bar "|."
        }
        \new Voice = "bass" {
          \voiceTwo
          \partial 4  e4
          dis e cis gis
          e fis b,\fermata e
          a gis8 [fis] gis [ais] b4
           fis fis, b,\fermata e
          a, b, cis8 [dis] e4
          b,2 e4 \bar "|."
          \partial 4 r
          dis2 e
          b, e
          fis4 gis a8 [gis fis e]
          dis4 e b,8 [b a gis]
          fis4 cis8 [b,] a,4 b,
          e2 cis4 dis
          eis8 [fisis] gis4 cis8 [dis] e4
          ais, b, e\fermata \bar "|."
        }
      >>
    }
  >>
}

\layout {
  \context {
    \Staff
    \consists "Merge_rests_engraver"
  }
}
\midi {}
