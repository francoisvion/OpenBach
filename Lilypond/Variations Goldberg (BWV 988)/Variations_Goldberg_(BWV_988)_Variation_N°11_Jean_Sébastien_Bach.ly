\version "2.24.4"
\paper {
    top-margin = 8\mm
    top-markup-spacing.basic-distance = #6
    markup-system-spacing.basic-distance = #5
    top-system-spacing.basic-distance = #12
    last-bottom-spacing.basic-distance = #12
    line-width = 18.0\cm
    ragged-bottom = ##f
    ragged-last-bottom = ##f
}
#(set-global-staff-size 19)
\header {
  title = "Variations Goldberg"
  subtitle = "Air avec 30 variations"
  piece = "Variation N°11, à 2 claviers"
  composer = "Jean-Sébastien Bach (1685-1750)"
  opus = "BWV 988"
  tagline = ##f
}
padFerm = \override Voice.Script.padding = #1
adjBeamOne = \once \override Beam.positions = #'( 0.3 . 3.2 )
adjBeamTwo = \once \override Beam.positions = #'( -1.8 . -0.2 )
adjBeamTre = \once \override Beam.positions = #'( -3.9 . -2.3 )
adjBeamQtr = \once \override Beam.positions = #'( -1.6 . -0.2 )
adjBeamCin = \once \override Beam.positions = #'( 3.7 . 1.6 )
adjBeamSix = \once \override Beam.positions = #'( 2.7 . 1.7 )
adjBeamSev = \once \override Beam.positions = #'( 1.4 . 0.3 )
adjBeamOct = \once \override Beam.positions = #'( 2.1 . 1.0 )
adjBeamNov = \once \override Beam.positions = #'( 0.5 . -0.3 )
adjBeamTen = \once \override Beam.positions = #'( 1.3 . 1.3 )
adjTieOne = \shape #'((0 . 0.2) (0 . 0.3) (0 . 0.3) (0 . 0.2)) Tie
adjTieTwo = \shape #'((0 . -0.2) (0 . -0.3) (0 . -0.3) (0 . -0.2)) Tie
adjTieTre = \shape #'((0.1 . -0.6) (0 . -1.0) (0 . -1.0) (0.1 . -0.6)) Tie
adjTieQtr = \shape #'((0.1 . 0.6) (0 . 1.0) (0 . 1.0) (0.1 . 0.6)) Tie
soprano =   \relative b' {
    \repeat volta 2 {
        b16 [ a g ] fis [ e d ] cis8. ~ [ cis8 e16 ] |
        a16 [ g fis ] e [ d cis ] \adjBeamOne b8. ~ [ b8 a'16 ] |
        g16 [ fis e ] d [ cis b ] a8 [ cis16] e8[ g16 ] |
        fis16 [ e d ] cis [ b a ]  \clef "bass" g [ fis e ] d [ e c! ] |
        b16 [ d g ] \adjBeamTwo b [ d g ] d [ b g ] f [ e d ] |
        \adjBeamTre cis16 [ e g ] \adjBeamQtr bes [ e g ] e [ bes a ] g [ fis e ] |
        d16 [ g b ]  d_[  \clef "treble" g b ] a [ fis d ] c [ b a ] |
        g16 [ b d ] g [ b d ] g8 [ d16 ] b8 [ fis16 ] |
        \adjTieTwo g8. ~ [ g8 b16 ] e [ d cis ] b [ a g ] |
        \adjTieTwo fis8. ~ [ fis8 e'16 ] d [ cis b ] a [ g fis ] |
        \adjTieTwo e8. ~ e16 [ fis g ] cis, [ b' a ] g [ a b ] |
        e,16_[ d' cis ] b [ cis d ] cis [ e fis ] g [ fis e ] |
        \adjTieOne d4. ~ \downprall d8_[ fis,16 ] a8 [ d16 ] |
        \adjTieOne d4. ~ \upprall d8 [ gis,16 ] b8 [ d16 ] |
        \adjTieOne d4. ~ \upprall d16 [ e g ] cis, [ e g ] |
        fis8. ~ fis16 [ d a ] fis [ d fis ] a [ cis d] |
    }
    \repeat volta 2 {
		r4. a'16 [ g fis ] e [ d c ] |
        b16 [ a g ] r8. d''16 [ c b ] a [ g f ] |
        e16 [ d c ] b [ a g ] fis8_[ dis'16 ] e8 [ ais,16 ] |
        b8 [ dis,16 ] fis8 [ a16 ] c^[ b a ] g [ a fis ] |
        e16 [ g b ] e [ g b ] g [ e b ] g [ fis e ] |
        f16 [ a c ] f [ a c ] a [ f c ] a [ g fis ] |
        g16 [ b e ] g [ e c' ] b [ a g ] fis [ e dis ] |
        e16 [ g e ] b [ g fis ] g [ e g ] b [ e dis ] |
        e16 [ d c ] b [ a g ] \adjTieTwo fis8. ~ [ fis8 a16 ] |
        d16 [ c b ] a [ g fis ] \adjTieTwo e8. ~ [ e8 d'16 ] |
        c16 [ b a ] g [ fis e ] fis [ e d ] cis [ d e ] |
        fis16 [ e d ] \clef "bass" c! [ b a ] b [ a g ] fis [ g a ] |
        b16 [ a g ] f [ e d ] e [ d c ] b [ c d ] |
        e16 [ c e ] g [ c e ] c [ a c ] \clef "treble" e [ a c ] |
        fis,16 [ d fis ] a [ d fis ] d [ a d ] fis [ a c ] |
        b16 [ g d ] b [ g fis ] g [ b d ] \padFerm g8.^\fermata |
    }
}
bass = \relative g {
	\repeat volta 2 {
        \adjTieOne g8. ~ [ g8 b16 ] e [ d cis ] b [ a g ] |
        fis8. ~ [ fis8 e'16 ] d [ cis b ] a [ g fis ] |
        e8 [ g16 b8 d16 ] cis [ b a ] g [ fis e ] |
        d16 [ e fis ] g [ a b ] \clef "treble" cis [ d e ] fis [ g a ] |
        \adjTieTwo g4. ~ \prallprall g8 [ b,16 ] d8 [ g16 ] |
        \adjTieTwo g4. ~ \upprall g8 [ cis,16 ] e8 [ g16 ] |
        \adjTieTwo g4. ~ \upprall g16 [ a c ] fis, [ a c ] |
        \stemUp \adjTieTwo  b8.~ \stemNeutral \adjBeamCin b16 [ g d ] b [ g b ] d [ g a ] |
        b16 [ a g ] fis [ e d ] cis8. ~ [ cis8 e16 ] |
        a16 [ g fis ] e [ d cis ] b8. ~ [ b8 a'16 ] |
        \adjBeamSix g16 [ fis e ] \adjBeamSev d [ cis b ] \adjBeamOct e [ d cis ] \adjBeamNov b [ a g ] \clef "bass" |
        cis16 [ b a ] g [ fis e ] a [ g fis ] e [ fis g ] |
        fis16 [ a d ] \clef "treble"  fis [ a d ] a [ fis d ] c [ b a ] |
        gis16 [ b d ] f [ b d ] b [ f e ] d [ cis b ] |
        a16 [ d fis! ] a [ d fis ] e [ cis a ] g [ fis e ] |
        d16 [ \clef "bass" a fis ] d [ a fis ] d [ fis a ] d8. |
    }
    \repeat volta 2 {
         d16 [ e fis ] g [ a b ] c [ d, c' ( ] c ) [ b a ] |
        g16 [ a b ] \clef "treble" c [ d e ] f [ g, f' ~ ] f [ e d ] |
        c8. ~ c16 [ e g ] c^[ b a ] g [ fis e ] |
        dis16 [ cis b ]  \clef "bass" a [ g fis ] e [ dis cis ] b [ c a ] |
        \adjTieTwo g8. ~ g16 [ b e ] g [ b dis ] \adjBeamTen e8 [ \stemUp  g,,16 ] \stemNeutral |
        \adjTieTwo a8. ~ a16 [ c f ] a [ c dis ] \adjBeamTen e8 [ \stemUp ais,,16 ] \stemNeutral |
        \adjTieTwo b8. ~ b16 [ cis dis ] e [ fis g ] a [ g fis ] |
        g16 [ e g ] b [ e dis ] e [ g e ] b [ g b ] |
        \stemUp \adjBeamTen \adjTieTre c,8. ~ [ c8 \stemDown e'16 ] \stemNeutral \clef "treble"  a [ g fis ] e [ d c ] |
        \adjTieTwo b8. ~ [ b8 a'16 ] g [ fis e ] d [ c b ] |
        \adjTieTwo a8. ~ a16 [ b c ] d [ e fis ] g [ fis e ] |
        \adjTieTwo d8. ~ d16 [ e fis ] g [ a b ] c^[ b a ] |
        \adjTieTwo g8. ~ g16 [ a b ] c [ d e ] f [ e d ] |
        \adjTieQtr c8. ~ c16 [ e c ] a [ e c ] a [ c e ] |
        a16 [ c a ] fis [ d a ] \clef "bass" fis [ d fis ] a [ c fis, ] |
        g16 [ b d ] \clef "treble"  g [ b d ] g [ d b ] \padFerm g8._\fermata
    }
}
\score {
    \context PianoStaff <<
        \set PianoStaff.midiInstrument = "harpsichord"
        \new Staff = "upper" { \clef "treble" \key g \major \time 12/16 \soprano  }
        \new Staff = "lower"  { \clef "bass" \key g \major \time 12/16 \bass }
    >>
    \layout{  }
    \midi { \tempo 4 = 93 }
}
